<template>
  <div class="app-container">
    <el-radio-group v-model="radio">
      ...
      <el-radio-button label="反义词"></el-radio-button>
    </el-radio-group>
    <h2>&nbsp;</h2>
    <div class="tip" v-if="radio==='同义词'">
      同义词查询:
    </div>
    <div class="tip" v-if="radio==='反义词'">
      反义词查询:
    </div>
    <el-input v-model="textarea" type="textarea" :disabled="stage" :rows="11" placeholder="请输入文本" clearable />

    <div style="text-align: center; padding-top:10px;padding-bottom:10px;">
      <el-upload ref="upload" action="/" accept=".txt" :before-upload="beforeUpload" :show-file-list="false"
        ...
        <el-button type="success" round @click="errorCorrect2()"  v-if="radio==='同义词'">同义词查询</el-button>
        <el-button type="success" round @click="errorCorrect()" v-if="radio==='反义词'">反义词查询</el-button>
  ...
  methods: {
    ...
    errorCorrect() {
      ...
      if (context === '') {
        ...
        axios.post('http://127.0.0.1:8000/v1/antonym', {
          text: that.textarea
        }).then((response) => {
    ...
    errorCorrect2() {
      ...
      if (context === '') {
        ...
        // 请求后端API服务，请求方法为post，请求体字段为json格式 text
        axios.post('http://127.0.0.1:8000/v1/synonym', {
          text: that.textarea
        }).then((response) => {
          console.log(response.data)
          that.result = response.data.correctionResults
